...
def redirect_view(request, *args, **kwargs):
    try:
        data = signing.loads(request.GET.get("data", ""), salt="plugins:saferpay:redirect:safe-redirect-data")
    ...
    if "go" in request.GET:
        if "session" in data:
            for k, v in data["session"].items():
                if k.startswith("payment_saferpay_"):
                    request.session[k] = v
        ...
        r = render(
            request,
            "pretix_saferpay/redirect.html",
            {
                "url": build_absolute_uri(
                    request.event, "plugins:pretix_saferpay:redirect"
                )
                + "?"
...
class ReturnView(SaferpayOrderView, View):
    ...
    def _redirect_to_order(self):
        ...
        if (
            self.request.session.get("payment_saferpay_order_secret")
            != self.order.secret
        ):
            messages.error(
                self.request,
